...
class Engtz_Global_Tranz_Fdo
{
     ...
     * @return array
     */
    public function engtz_package_hazardous($package, $en_fdo_meta_data)
    {
     ...
     * @return array
     */
    public function engtz_cart_package($package)
    {
        $this->en_fdo_meta_data['plugin_type'] = 'ltl';
        $this->en_fdo_meta_data['plugin_name'] = 'cerasis_freights';
        $accessorials['notify'] = get_option('wc_settings_cerasis_notify') == 'yes' ? true : false;
        $accessorials['residential'] = get_option('wc_settings_cerasis_residential_delivery') == 'yes' ? true : false;
        $accessorials['liftgate'] = get_option('wc_settings_cerasis_lift_gate_delivery') == 'yes' ? true : false;
        $accessorials['limitedaccess'] = get_option('gtz_limited_access_delivery') == 'yes' ? true : false;
        $this->en_fdo_meta_data['accessorials'] = $accessorials;

        (isset($package['items'])) ? $this->engtz_package_items($package['items']) : '';
        (isset($package['origin'])) ? $this->engtz_package_address($package['origin']) : '';
     ...
     * @param type $items
     */
    public function engtz_package_items($items)
    {
        $this->en_fdo_meta_data['items'] = [];
        foreach ($items as $item_key => $item_data) {
            $nmfc_number = $productId = $productName = $productQty = $actualProductPrice = $products = $productPrice = $productWeight = $productLength = $productWidth = $productHeight = $ptype = $hazardousMaterial = $productType = $productSku = $productClass = $attributes = $variantId = '';
            $attributes = [];
            extract($item_data);
     ...
     * @param array $address
     */
    public function engtz_package_address($address)
    {
    ...
    public function get_fdo_coupon_data_from_api(){
        ...
        $url = GT_FDO_COUPON_BASE_URL . "/use_coupon";
    ...
    public function get_fdo_coupon_parsed_data($fdo_coupon_data)
    {
        ...
        $parsed_data['register_url'] = GT_FDO_COUPON_BASE_URL.'/register?code='.$base64string;
        $parsed_data['login_url'] = GT_FDO_COUPON_BASE_URL.'/login?code='.$base64string;
        // $parsed_data['user_guide_get_started'] = GT_FDO_COUPON_BASE_URL.'/help/Getting_Started';
