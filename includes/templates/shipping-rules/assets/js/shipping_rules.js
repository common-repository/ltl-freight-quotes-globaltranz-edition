jQuery(document).ready(function () {
	const form = jQuery('#gtz_ltl_add_shipping_rule');
	if (form) {
		form.on('submit', enGtzLtlSaveShippingRule);
	}

	enGtzLtlToggleRuleTypeFields();
	if (window.location.hash.includes('en_gtz_ltl_add_sr_btn')) window.location.hash = '';

	// #region: Add Shipping Rule
	jQuery('#en_gtz_ltl_add_shipping_rule_btn').on('click', function () {
		form[0]?.reset();
	...
	jQuery('#en_gtz_ltl_sr_rule_type').on('load change', function (e) {
		enGtzLtlToggleRuleTypeFields();
	});
	...
	jQuery(document).on('click', '.en_gtz_ltl_sr_edit_link', function () {
		const id = jQuery(this).attr('data-id');
		enGtzLtlEditShippingRule(id, this);
	});
	jQuery(document).on('click', '.en_gtz_ltl_sr_delete_link', function () {
		const id = jQuery(this).attr('data-id');
		enGtzLtlDeleteShippingRule(id, this);
	});
	jQuery(document).on('click', '.en_gtz_ltl_sr_status_link', function (e) {
		enGtzLtlUpdateShippingRuleStatus(this);
	});
	...
	function enGtzLtlSRValidateInput(form_id = '') {
		let is_valid = true;
		...
		if (jQuery('#en_gtz_ltl_sr_rule_type').val() === 'Override Rates') {
	...
	function enGtzLtlSaveShippingRule(e) {
		e.preventDefault();

		const is_valid = enGtzLtlSRValidateInput('#gtz_ltl_add_shipping_rule');
		if (!is_valid) {
		...
		const postData = {
			action: 'en_gtz_ltl_save_shipping_rule',
			rule_id: jQuery('#edit_sr_form_id').val(),
			name: jQuery('#en_sr_rule_name').val(),
			type: jQuery('#en_gtz_ltl_sr_rule_type').val(),
		...
		jQuery.ajax({
			...
			dataType: 'json',
			beforeSend: function () {
				jQuery('.en_gtz_ltl_save_shipping_rule_form').addClass('spinner_disable').val('Loading...');
			},
			success: function (data) {
				jQuery('.en_gtz_ltl_save_shipping_rule_form').removeClass('spinner_disable').val('Save');
				const rowMarkup = enGtzLtlGetRowMarkup({ ...data, ...postData });
				jQuery('.en_gtz_empty_row').hide();

				if (data.insert_qry == 1) {
					const newRowMarkup = `<tr id='sr_row_${data.id}' class='en_gtz_sr_row'>${rowMarkup}</tr>`;
					jQuery('#en_gtz_shipping_rules_list tbody').append(newRowMarkup);
					window.location.href = jQuery('.close').attr('href');
					jQuery('.gtz_ltl_sr_created').show('slow').delay(3000).hide('slow');
				} else if (data.update_qry == 1) {
					jQuery(`#sr_row_${data.id}`).html(rowMarkup);
					window.location.href = jQuery('.close').attr('href');
					jQuery('.gtz_ltl_sr_updated').show('slow').delay(3000).hide('slow');
				} else if (data.error) {
					jQuery('.gtz_ltl_sr_already_exist').show('slow');
					jQuery('.content').delay(200).animate({ scrollTop: 0 }, 300);
					setTimeout(function () {
						jQuery('.gtz_ltl_sr_already_exist').hide('slow');
					}, 3000);
				}
			},
	...
	if (typeof enGtzLtlEditShippingRule != 'function') {
		function enGtzLtlEditShippingRule(rule_id, elem) {
			form[0]?.reset();
			jQuery('.en_sr_err').html('');
			setDataAttribute(
				'en_sr_weight_from',
			...
			const postForm = {
				action: 'en_gtz_ltl_edit_shipping_rule',
				edit_id: rule_id,
			};
			...
			jQuery.ajax({
				...
				success: function (data) {
					...
					if (data) {
						...
						jQuery('#en_gtz_ltl_sr_rule_type').val(data?.rule_data?.type);
						...
						// Show popup modal
						window.location.href = jQuery('.en_gtz_ltl_add_sr_btn').attr('href');
						setTimeout(function () {
							if (jQuery('.en_gtz_ltl_add_sr_popup').is(':visible')) {
	...
	if (typeof enGtzLtlDeleteShippingRule != 'function') {
		function enGtzLtlDeleteShippingRule(id, elem) {
			const postForm = {
				action: 'en_gtz_ltl_delete_shipping_rule',
				delete_id: id,
			};
			...
			jQuery.ajax({
				...
				success: function (data) {
					...
					jQuery('.gtz_ltl_sr_deleted').show('slow').delay(3000).hide('slow');
					if (jQuery('tr.en_gtz_sr_row')?.length) return false;

					if (jQuery('.en_gtz_empty_row')?.length) {
						jQuery('.en_gtz_empty_row').show();
					} else {
						const newRowMarkup = `<tr class="new_warehouse_add en_gtz_empty_row"><td class="engtz_wd_warehouse_list_heading_data" colspan="5" style="text-align: center;">No data found!</td></tr>`;
						jQuery('#en_gtz_shipping_rules_list tbody').append(newRowMarkup);
					}
				},
				error: function (error) {},
	...
	function enGtzLtlUpdateShippingRuleStatus(elem) {
		const rule_id = jQuery(elem).attr('data-id');
		const is_active = jQuery(elem).attr('data-status');

		// Submit the form to save settings
		const postData = {
			action: 'en_gtz_ltl_update_shipping_rule_status',
			rule_id,
	...
	function enGtzLtlGetRowMarkup(rule) {
		const rowMarkup = `
				<td class="engtz_wd_warehouse_list_heading_data">${rule?.name}</td>
				<td class="engtz_wd_warehouse_list_heading_data">${rule?.type}</td>
				<td class="engtz_wd_warehouse_list_heading_data">${rule?.settings?.filter_name}</td>
				<td class="engtz_wd_warehouse_list_heading_data"><a href="#" class='en_gtz_ltl_sr_status_link' data-id='${
					rule?.id
				}' data-status='${rule?.is_active}'>${rule?.is_active ? 'Yes' : 'No'}</a></td>
				<td class="engtz_wd_warehouse_list_heading_data">
					<!-- Edit rule link -->
					<a href="#" class="en_gtz_ltl_sr_edit_link" data-id="${rule?.id}">
						<img src="${
							script.pluginsUrl
						}/ltl-freight-quotes-globaltranz-edition/includes/warehouse-dropship/wild/assets/images/edit.png" title="Edit">
					</a>
					<!-- Delete rule link -->
					<a href="#" class="en_gtz_ltl_sr_delete_link" data-id="${rule?.id}">
						<img src="${
							script.pluginsUrl
						}/ltl-freight-quotes-globaltranz-edition/includes/warehouse-dropship/wild/assets/images/delete.png" title="Delete">
					</a>
				</td>
		`;
	...
	function enGtzLtlToggleRuleTypeFields() {
		const type = jQuery('#en_gtz_ltl_sr_rule_type').val();
		jQuery('#en_gtz_ltl_sr_content').css('height', '80vh');
		...
		if (type === 'Hide Methods') {
			...
			jQuery('#gtz_ltl_add_shipping_rule').css('overflow', 'hidden');
			jQuery('#en_gtz_ltl_sr_content').css('height', '450px');
