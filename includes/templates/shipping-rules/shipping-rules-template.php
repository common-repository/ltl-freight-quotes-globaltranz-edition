 ...
 */
if (!function_exists('en_gtz_ltl_shipping_rules_template')) {
    function en_gtz_ltl_shipping_rules_template($action = false)
    {
      ob_start();

      global $wpdb;
      $shipping_rules_list = $wpdb->get_results(
          "SELECT * FROM " . $wpdb->prefix . "eniture_gtz_ltl_shipping_rules"
      );

      ?>
        <div>
          <table class="engtz_wd_warehouse_list_heading" id="en_gtz_shipping_rules_list">
            <!-- Table Headings -->
            <thead>
                  <tr>
              ...
              <?php
                if (count($shipping_rules_list) > 0) {
                  ...
                  foreach ($shipping_rules_list as $rule) {
                    ...
                    ?>
                      <tr id="sr_row_<?php echo (isset($rule->id)) ? esc_attr($rule->id) : ''; ?>" class="en_gtz_sr_row">
                        <td class="engtz_wd_warehouse_list_heading_data"><?php echo $rule->name; ?></td>
                        <td class="engtz_wd_warehouse_list_heading_data"><?php echo $rule->type; ?></td>
                        <td class="engtz_wd_warehouse_list_heading_data"><?php echo isset($rule->settings['filter_name']) ? $rule->settings['filter_name'] : ''; ?></td>
                        <td class="engtz_wd_warehouse_list_heading_data">
                          <a href="#" class="en_gtz_ltl_sr_status_link" data-id="<?php echo (isset($rule->id)) ? esc_attr($rule->id) : ''; ?>" data-status="<?php echo $rule->is_active; ?>"><?php echo $rule->is_active ? 'Yes' : 'No'; ?></a>
                        </td>
                        <td class="engtz_wd_warehouse_list_heading_data">
                          <!-- Edit rule link -->
                          <a href="#" class="en_gtz_ltl_sr_edit_link" data-id="<?php echo (isset($rule->id)) ? esc_attr($rule->id) : ''; ?>">
                            <img src="<?php echo ENGTZ_DIRPATH; ?>/includes/warehouse-dropship/wild/assets/images/edit.png" title="Edit">
                          </a>
                          <!-- Delete rule link -->
                          <a href="#" class="en_gtz_ltl_sr_delete_link" data-id="<?php echo (isset($rule->id)) ? esc_attr($rule->id) : ''; ?>">
                            <img src="<?php echo ENGTZ_DIRPATH; ?>/includes/warehouse-dropship/wild/assets/images/delete.png" title="Delete">
                          </a>
                        </td>
                      </tr>
                    <?php
                  ...
                  ?>
                    <tr class="new_warehouse_add en_gtz_empty_row">
                      <td class="engtz_wd_warehouse_list_heading_data" colspan="5" style="text-align: center;">
                        No data found!
                      </td>
                    </tr>
                  <?php
...
<div class="en_gtz_ltl_shipping_rules_setting_section">
    <br />
    <!-- Add rule button -->
    <div class="en_sr_add_btn">
      <a href="#en_gtz_ltl_add_sr_btn" title="Add Rule"
             class="en_gtz_ltl_add_sr_btn button-primary is-primary" id="en_gtz_ltl_add_shipping_rule_btn">Add Rule</a>
    </div>

    <div class="updated inline warehouse_deleted gtz_ltl_sr_deleted">
      <p><strong>Success!</strong> Shipping rule is deleted successfully.</p>
    </div>
    <div class="updated inline warehouse_created gtz_ltl_sr_created">
        <p><strong>Success!</strong> Shipping rule is added successfully.</p>
    </div>
    <div class="updated inline warehouse_updated gtz_ltl_sr_updated">
        <p><strong>Success!</strong> Shipping rule is updated successfully.</p>
    </div>

    <!-- Shipping rules data table -->
    <?php en_gtz_ltl_shipping_rules_template(); ?>

    <!-- Add popup for new rule -->
    <div id="en_gtz_ltl_add_sr_btn" class="engtz_wd_warehouse_overlay">
        <div class="engtz_wd_add_warehouse_popup en_gtz_ltl_add_sr_popup">
            <h2 class="warehouse_heading">Add Rule</h2>
            <a class="close" href="#">&times;</a>
            <div class="content" id="en_gtz_ltl_sr_content" style="overflow-y: auto; height: 80vh;">

              <div class="already_exist gtz_ltl_sr_already_exist">
                <strong>Error!</strong> Shipping rule with this name already exists.
              </div>
                <!-- Wordpress Form closed -->
                </form>

                <form id="gtz_ltl_add_shipping_rule" role="form">
                    ...
                    <!-- Type -->
                    <div class="en_sr_form_control">
                        <label for="en_gtz_ltl_sr_rule_type">Type</label>
                        <select name="en_gtz_ltl_sr_rule_type" id="en_gtz_ltl_sr_rule_type">
                    ...
                    <div class="en_sr_override_rates">
                      ...
                      <div class="en_sr_form_control">
                          ...
                          <select name="en_sr_service" id="en_sr_service">
                            ...
                            <?php 
                              $services = ['transportation_service' => 'Transportation service'];
                              $additional_services = ['residential_delivery_service' => 'Residential delivery service', 'liftgate_delivery_service' => 'Lift gate delivery service', 'notify_before_delivery_service' => 'Notify before delivery service'];
                              get_option('cerasis_global_tranz_api_endpoint') != 'wc_cerasis_api_fields' && $services = array_merge($services, $additional_services);
                    ...
                    <!-- Form submit button -->
                    <div class="form-btns">
                        <input type="submit" name="en_wd_submit_warehouse" value="Save" class="save_warehouse_form en_gtz_ltl_save_shipping_rule_form">
                    </div>
                </form>
            </div>
        </div>
