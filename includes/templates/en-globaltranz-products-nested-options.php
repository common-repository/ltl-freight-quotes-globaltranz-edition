...
if (!class_exists("EnUpdateProductNestedOptions")) {
    ...
    class EnUpdateProductNestedOptions {
        ...
        public function __construct($action) {
            ...
            if ($action == 'hooks') {
                $this->engtz_add_nested_simple_product_hooks();
                $this->engtz_add_variable_nested_product_hooks();
            }
        }
         ...
         */
        public function engtz_add_nested_simple_product_hooks() {

            /* Add simple product fields */
            add_action(
                    'woocommerce_product_options_shipping', array($this, 'engtz_show_nested_product_fields'), 110
            );
            add_action(
                    'woocommerce_process_product_meta', array($this, 'engtz_save_nested_product_fields'), 20
            );
        }
         ...
         */
        public function engtz_add_variable_nested_product_hooks() {

            add_action(
                    'woocommerce_product_after_variable_attributes', array($this, 'engtz_show_nested_product_fields'), 110, 3
            );

            add_action(
                    'woocommerce_save_product_variation', array($this, 'engtz_save_nested_product_fields'), 20
            );
        }
         ...
         * @param int $post_id
         */
        function engtz_save_nested_product_fields($post_id) {
...
//              add / update Max. Nested Items
         ...
         */
        function engtz_show_nested_product_fields($loop, $variation_data = array(), $variation = array()) {

            if (!empty($variation) || isset($variation->ID)) {
                /* Variable products */
                $this->engtz_product_custom_nested_fields($variation->ID);
            } else {
                /* Simple products */
                $post_id = get_the_ID();
                $this->engtz_product_custom_nested_fields($post_id);
            }
        }
         ...
         * @param $variation
         */
        function engtz_product_custom_nested_fields($post_id) {
            ...
            if (isset($plan_status['nesting'])) {
                if (!in_array(0, $plan_status['nesting']['plan_required'])) {
                    ...
                    $description = apply_filters("globaltranz_plans_notification_link", [3]);
