...
if (!class_exists("Engtz_Woo_Wd_Addons_Ajax_Req_Includes")) {

    class Engtz_Woo_Wd_Addons_Ajax_Req_Includes
    {
        ...
        {
            add_action('wp_ajax_nopriv_engtz_wd_get_address', array($this, 'get_address_api_ajax'));
            add_action('wp_ajax_engtz_wd_get_address', array($this, 'get_address_api_ajax'));
            ...
            add_action('wp_ajax_nopriv_engtz_wd_save_warehouse', array($this, 'save_warehouse_ajax'));
            add_action('wp_ajax_engtz_wd_save_warehouse', array($this, 'save_warehouse_ajax'));

            add_action('wp_ajax_nopriv_engtz_wd_save_dropship', array($this, 'save_dropship_ajax'));
            add_action('wp_ajax_engtz_wd_save_dropship', array($this, 'save_dropship_ajax'));


            add_action('wp_ajax_nopriv_engtz_wd_edit_dropship', array($this, 'edit_dropship_ajax'));
            add_action('wp_ajax_engtz_wd_edit_dropship', array($this, 'edit_dropship_ajax'));
            ...
            add_action('wp_ajax_nopriv_engtz_wd_edit_warehouse', array($this, 'edit_warehouse_ajax'));
            add_action('wp_ajax_engtz_wd_edit_warehouse', array($this, 'edit_warehouse_ajax'));
        }
        ...
        function get_address_api_ajax()
        {
            if (isset($_POST['origin_zip'])) {
                ...
                $accessLevel = 'address';
                $get_sm_distance = new Engtz_Get_cerasis_freight_distance();
                ...
                if (count($map_result['results']) > 0) {
                    ...
                    if ($map_result['results'][0]['postcode_localities']) {
                        ...
                        $city = '<select id="' . $address_type . '_city" class="city-multiselect select engtz_wd_multi_state city_select_css" name="' . $address_type . '_city" aria-required="true" aria-invalid="false">
                                    ' . $city_option . '</select>';
        ...
        function save_warehouse_ajax()
        {
            ...
            $input_data_arr = array(
                ...
                'origin_markup' => isset($inputData['origin_markup']) ? sanitize_text_field($inputData['origin_markup']) : '',
                ...
                if (isset($validateData["city"])) {
                    ...
                    if ($get_warehouse_id && (empty($get_warehouse) || (!empty($get_warehouse) && reset($get_warehouse)->id == $get_warehouse_id))) {
                        ...
                        if (empty($get_warehouse)) {
                            ...
                            $html = engtz_warehouse_template(TRUE);
                        }
                    }
                }
                ...
                if (isset($validateData["city"])) {
                    ...
                    if ($get_dropship_id != '' && (empty($get_dropship) || (!empty($get_dropship) && reset($get_dropship)->id == $get_dropship_id))) {
                        ...
                        if (empty($get_dropship)) {
                            ...
                            $html = engtz_dropship_template(TRUE);
                        }
                    }
                }
                $lastid = $wpdb->insert_id;
        ...
        function delete_dropship_ajax()
        {
            ...
            $get_dropship_id = (isset($_POST['dropship_delete_id']) && intval($_POST['dropship_delete_id'])) ? intval($_POST['dropship_delete_id']) : "";
            $get_dropship_array = array(wp_unslash($_POST['dropship_delete_id']));
            $get_dropship_val = array_map('intval', $get_dropship_array);
            $ser = serialize($get_dropship_val);
            $get_post_id = $wpdb->get_results("SELECT group_concat(post_id) as post_ids_list FROM `" . $wpdb->prefix . "postmeta` WHERE `meta_key` = '_dropship_location' AND (`meta_value` LIKE '%" . $ser . "%' OR `meta_value` = '" . intval($_POST['dropship_delete_id']) . "')");
...
new Engtz_Woo_Wd_Addons_Ajax_Req_Includes();
